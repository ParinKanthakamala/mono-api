...
namespace Tools.Label
{
    ...
    {
        public EmbeddedResourceKeysProvider(Assembly assembly, string resourceFolderName = "Resources") : base(assembly,
            resourceFolderName)
        {
        }
        ...
        protected override string[] GetLanguageFileNames()
        {
            var languageFileNames = _resourcesAssembly.GetManifestResourceNames().Where(s =>
                    s.Contains(ResourceFolderName) && (s.Contains(".yml") || s.Contains(".yaml")) && s.Contains("-"))
                .ToArray();
        ...
        protected override string GetFileName(string cultureName)
        {
            var fileName = _resourcesAssembly.GetManifestResourceNames().SingleOrDefault(s =>
                s.Contains(ResourceFolderName) &&
                (s.Contains($"{cultureName}.yml") || s.Contains($"{cultureName}.yaml")));
            return fileName;
        }
